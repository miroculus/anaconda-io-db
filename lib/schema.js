...
module.exports = (schemaDefinition = {}) => {
  ...
  const self = {
    ...
    /**
     * Create a document, the primaryKey is optional
     * @param {object} attrs
     */
    create: async (attrs) => {
      ...
      // Add a custom UUID
      if (!data[primaryKey] && primaryKeyType === 'STRING') {
    ...
    /**
     * Update one or more documents on the table
     * @param {object} query query to filter docs to be updated
     * @param {object} changes attributes to update
     * @returns {Promise<object[]>} array of the resulting edited documents
     */
